...
pacman::p_load(reshape2, naniar, actuar, dplyr, tidyr, kableExtra, knitr, LearnBayes, 
               ...
               devtools, ggbiplot, caret, standardize, EBglmnet, car)
```
...
ggplot() + 
  geom_point(aes(x=sc.set.limit$component_density,
                 y=sc.set.original$ten_inf_ind), shape = ".") +
...
Now, I will outline the specifics of the models I investigated for this project. All that we have left to decide is our prior for $\sigma^2$ and our choice of hyerparameter $\lambda$. 
...
Additionally, T. Park and G. Casella [Journal of the American Statistical Association, 2008] suggest using an inverse gamme prior distribution because it is a conjugate prior. Where $\{\sigma^2,a,\gamma\}>0$,

$$p(\sigma^2) = \frac{\gamma^a}{\Gamma(a)}(\sigma^2)^{-a-1}\exp(\frac{-\gamma}{\sigma^2})$$

For simplicities sake here, I will choose to use the scale invariant prior. T. Park and G. Casella argue that under basic assumptions the two are relatively similar. The resulting posteriors take similar for and are both proper. The posterior when using the scale invariant prior follows:

$$p(\boldsymbol{\beta},\sigma^2|\tilde{\textbf{y}}) \propto (\sigma^2)^{-(n+p-1)} \exp\left\{-\frac{1}{2\sigma^2}(\tilde{\textbf{y}}-\textbf{X}\boldsymbol{\beta})^T(\tilde{\textbf{y}}-\textbf{X}\boldsymbol{\beta}) +\lambda\sum_{i=1}^p|\beta_j|\right\}$$
...
Our first model of interest is the Frequentist Lasso model. I used the "glmnet" function to calculate the coefficients resulting from a the L1 Norm penalty term defined at the start of the Methods section. I had used this model before, so I was interested to see if there were any differences between the selection made from the typical frequentist method and those under bayesian frameworks. I do not expect there to be substantial difference in which variable were selected.**add here? Please see the selected $\beta$ coefficients and their estimates below (Table 1). All estimates above 0.1 are reported.  
...
x <- model.matrix(cumulative_incidence~.,data=sc.set.cum.inc)[,-1]
...
out.lasso=glmnet((model.matrix(cumulative_incidence~.,data=sc.set.cum.inc)[,-1]),
...
lasso.coef.non.zero <- lasso.coef[lasso.coef>0.01]
...
rownames(lasso.coef.non.zero.df) <- c("II Closeness Centrality",
                                      "II Eigenvector Centrality", "Component Size",
                                      "Component Density", "Transitivity", "Diameter")
...
Next, I constructed a Bayesian Lasso using STAN. A package called "rstanarm" allowed me to quickly employ Hamiltonian Monte Carlo (HMC) Sampling. I used the default number of chains (4), burn in iterations (1,000), and the number of interations for progression (1,000). For this model I also employed cross validation (cv.EBglmnet from the "EBglmnet" package) to determine the value of $\lambda$. The model chooses the lambda that minimizes the sqrare error. The $\beta$ coefficients of the selected variables are displayed below (Table 2).
...
out.glm_post_cvlam <- summary(glm_post_cvlam)[1:10,c(3:6,8)]
...
rownames(out.glm_post_cvlam) <- c("Intercept", "Maximum Degree", 
                                  ...
                                  "Component Density", "Geodesic Distance")
...
Our last model will employ a gamma distribution as a diffuse hyperprior for $\lambda$. You can see the output and trace plots below (Table 3). 

```{r, CI Bayesian Lasso, Diffuse Hyperprior Lambda, eval = T, results='hide', message=F, fig.keep='all'}
############################################
# CI Bayesian Lasso, Diffuse Hyperprior Lambda
glm_post_hypprior.lam <- stan_glm(cumulative_incidence ~ ., 
                                  data = sc.set.cum.inc, 
                                  prior = lasso(df = 1, location = 0, 
                                                scale = NULL, autoscale = TRUE),
                                  prior_intercept = NULL)
#summary(glm_post_hypprior.lam)
```

```{r, eval=T, results='asis', message=F, fig.keep='all'}
############################################
# Build and Print Table
out.glm_post_hypprior.lam <- summary(glm_post_hypprior.lam)[1:10,c(3:6,8)]
out.glm_post_hypprior.lam <- out.glm_post_hypprior.lam[, c(2, 3, 4, 1, 5)]
out.glm_post_hypprior.lam <- data.frame(round(out.glm_post_hypprior.lam, 2))
colnames(out.glm_post_hypprior.lam) <- c("10%", "50%", "90%", "Std Dev", "Rhat")
rownames(out.glm_post_hypprior.lam) <- c("Intercept", "Maximum Degree", 
                                         "Betweenness Centrality", 
                                         "II Betweenness Centrality", 
                                         "II Closeness Centrality", 
                                         "II Eigenvector Centrality", 
...
kable(out.glm_post_hypprior.lam, "latex", 
      caption = "Bayesian Lasso, Cross Validated Lambda", booktabs = T) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```

The model appears to be converging nicely, all four chains are similar across each predictor, no matter if the predictor's posterior is pulled away from zero or remains null. The tables above also display the $\hat{R}$ values, which all reach 1.

```{r, Trace Plots, eval=T, fig.width=8.5}
############################################
# Look at trace plots
stan_trace(glm_post_hypprior.lam)
```
...
############################################s
# Posterior Predictive Checks
pp_check(glm_post_hypprior.lam)
```
...
############################################
#  Posterior Predictive Checks for Bayesian Lasso, Diffuse Hyperprior Lambda
stan_hist(glm_post_hypprior.lam, pars=c("maximum_degree","betweenness_centrality",
                                        "ii_betweenness_centrality",
                                        "ii_closeness_centrality",
                                        "ii_eigenvector_centrality", "component_size"),
          bins=40, fill_color="blue")

stan_hist(glm_post_hypprior.lam, pars=c("number_connected_components",
                                        "component_density",
                                        "geodesic_dist","centralization", 
                                        "prop_2_cores", "diameter"), 
          bins=40, fill_color="blue")

#post_samps_speed <- as.data.frame(glm_post_hypprior.lam, pars=c("geodesic_dist"))[,"geodesic_dist"]
